...
## Objective
Expand the Home Assistant Todoist integration so automations and blueprints can interact with richer task metadata, leverage newer API options, and optionally return complete task payloads.

## Key Changes Needed
1. **API surface enrichment**
	- Adopt the latest `todoist_api_python` async models to expose additional fields supplied by `/tasks/get` (labels, assignee, editor, section, recurring schedule, etc.).
	- Normalize field names in `types.py` (or equivalent data classes) to match the SDK models for round-trip serialization.
2. **Coordinator & data model updates**
	- Extend the `TodoistCoordinator` to request expanded task objects (pass `include_history`, `lang`, and other supported query params when beneficial).
...
3. **Service layer improvements**
	...
	- Ensure service calls return the full task dictionary when configured (consider a `return_full_task` toggle in options).
...
## Deployment Strategy
- Initially distribute as a `custom_components/todoist` drop-in replacement. Provide instructions for removing the stock integration before enabling the custom one.
